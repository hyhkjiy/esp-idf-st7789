...
extern const uint8_t img_1[]   asm("_binary_1_jpg_start");
extern const uint8_t img_2[]   asm("_binary_2_jpg_start");
...
TickType_t ImageTest(TFT_t *dev, int width, int height){
    TickType_t startTick, endTick, diffTick;
    startTick = xTaskGetTickCount();

    lcdDrawImage(dev, img_2, 0, 0, 240, 240);
...
void ST7789(void *pvParameters) {
    ...
    while (1) {
        ...
        ImageTest(&dev, CONFIG_WIDTH, CONFIG_HEIGHT);
        WAIT;
        ...
        lcdSetFontDirection(&dev, 0);
        WAIT;
        ESP_LOGI(TAG, "Free memory size: %d", heap_caps_get_free_size(MALLOC_CAP_DEFAULT));
